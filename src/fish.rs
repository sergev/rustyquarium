...
pub fn add_bubble(fish: EntityRef, anim: &mut Animation) {
    let (bx, by, fz, fw, fh) = {
        ...
        let bx = if let CallbackArgs::Move(ref v) = f.callback_args {
            if v.first().copied().unwrap_or(0.0) > 0.0 { fx + fw as i32 } else { fx }
        } else {
            fx
        };
...
pub fn fish_collision(fish: EntityRef, anim: &mut Animation) {
    ...
    for obj in &collision {
        ...
        if obj_type == "hook_point" {
            ...
            for h in anim.get_entities_by_type("fishhook") { retract(h, anim); }
            for l in anim.get_entities_by_type("fishline")  { retract(l, anim); }
            return;
        }
    }
}
...
mod tests {
    ...
    fn test_rand_color_replaces_digits_with_valid_color_chars() {
        ...
        assert_eq!(mask.chars().count(), 9, "length should be preserved after digit replacement");
    ...
    fn test_rand_color_leaves_non_digits_unchanged() {
        ...
        assert_eq!(mask, "abc!@#", "non-digit chars must pass through unchanged");
    }
    ...
    fn test_add_fish_always_has_valid_shape() {
        for classic in [false, true] {
            let mut a = make_anim();
            for i in 0..200usize {
                let before: HashSet<*const _> =
                    a.entities.iter().map(|e| Rc::as_ptr(e)).collect();
                add_fish(None, &mut a, classic);
                let fish = a.entities.iter()
                    .find(|e| !before.contains(&Rc::as_ptr(e)))
                    .cloned()
                    .unwrap_or_else(|| panic!("classic={} iter {}: add_fish added no new entity", classic, i));
                let b = fish.borrow();
                assert_eq!(b.entity_type, "fish",
                    "classic={} iter {}: entity_type should be \"fish\"", classic, i);
                assert!(!b.current_shape().is_empty(),
                    "classic={} iter {}: fish entity has empty shape", classic, i);
            }
        }
    }
    ...
    fn test_fish_shape_not_corrupted_by_color_arg() {
        for i in 0..500usize {
            ...
            assert_eq!(e.borrow().current_shape(), shape_val,
                "iter {}: fish shape corrupted after rand_color allocation", i);
        }
    }
}
