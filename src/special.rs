...
mod tests {
    ...
    fn test_sea_monster_sprites_parity() {
        for _ in 0..40 {
            ...
            assert!(
                e.shapes.len() >= 2,
                "monster must have ≥2 animation frames, got {}",
                e.shapes.len()
            );
            ...
            assert!(
                !e.colors.is_empty() && !e.colors[0].is_empty(),
    ...
    fn test_big_fish_visual_parity() {
        for _ in 0..30 {
            ...
            if let CallbackArgs::Move(ref v) = e.callback_args {
                ...
                assert!(
                    speed == 2.5 || speed == 3.0,
                    "big fish speed must be 2.5 or 3.0, got {}",
                    speed
                );
            } else {
    ...
    fn test_shark_and_teeth_parity() {
        ...
        assert!(
            !s.current_color().is_empty(),
            "shark must have a color mask"
        );
        ...
        assert!(
            tx == -9 || tx == sx + 9,
            "unexpected teeth x: shark_x={} teeth_x={}",
            sx,
            tx
        );
    }
    ...
    fn test_fishhook_visual_parity() {
        ...
        assert!(
            hook_shape.split('\n').count() >= 6,
            "fishhook must have ≥6 lines, got {}",
            hook_shape.split('\n').count()
        );
        ...
        assert!(
            pipe_segments >= 50,
            "fishline must have ≥50 pipe segments, got {}",
            pipe_segments
        );
    }
    ...
    fn test_surface_sprites_use_real_multiline_strings() {
        ...
        for e in &a.entities {
            let b = e.borrow();
            if !surface_depths.contains(&b.z) {
                continue;
            }
            let shape = b.current_shape();
            assert!(
                shape.contains('\n'),
                "surface sprite {:?} at z={} must be multiline",
                b.entity_type,
                b.z
            );
            assert!(
    ...
    fn test_fishhook_lowers_in_lowering_mode() {
        ...
        assert!(
            y_after > y_before,
            "fishhook should descend in lowering mode (y_before={} y_after={})",
            y_before,
            y_after
        );
    }
